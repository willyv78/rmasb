(function(g, h, e)
{
    ...
    g.Elastislide.defaults = {
        ...
        speed: 500,
        easing: "ease-in-out",
        minItems: 2,// PROPIEDAD QUE DETERMINA EL NÚMERO DE IMAGENES DE PASO EN LA GALERÍA
        start: 0,
...
$(function()
{
    ...
    $.fn.elastic_grid = function(G)
    {
        ...
        var carrusel_container = $("<div></div>");
        var carrusel_slider = $("<div></div>");
        var carrusel_loading = $("<div></div>");
        var carrusel_navigator = $("<div></div>");
        carrusel_container.appendTo(carrusel_loading);
        carrusel_container.appendTo(carrusel_slider);
        carrusel_container.appendTo(carrusel_navigator);
        //alert(u.attr("id"));
        if (H == 0)
        {
            return false
        ...
        x.each(function(J)
        {
            ...
            //if(num_li > 5){alert("muy grande");}
            if(numOfTag > 0){K.addClass("hidden");}
        });
        //alert("cantidad " + numOfTag);
        if (numOfTag > 1)
        {
            f(G.showAllText);
            $.each(c, function(I, i)
        ...
        {   
            ...
            });//evento determina cuando se le da clic k al primer menú
            return false
        });

        function p(i, I)//permite la elección del submenú
        {
            i.each(function(J, K)
            {
                if (classie.has(K, I))
        ...
        m.find("li:first").addClass("current"); //selección por defecto del primer elemento pulsado en la galería menú principal
        ...
        e.prototype = {
            create: function()
            {
                ...
                this.$closePreview = $('');//<span class="og-close"></span> --> SPAN QUE PERMITE EL CIERRE DE PESTAÑA DE GALERÍA
            ...
            update: function(L)
            {
                ...
                else
                {
                    ...
                    if (urlList.length > 0)
                    {
                        ...
                        $(".link-button").on("click", mostrarSection);
                    }
                    var O = this;
                    ...
                    if (glarge.length == gthumbs.length && glarge.length > 0)
                    {
                        ...
                        for (C = 0; C < gthumbs.length; C++)
                        {
                            ...
                            var J = $("<img/>");
                            J.addClass("related_photo"); // ESTE ELEMENTO BLOQUEA LA GALERÍA NO DEJA EXPANDIRLAS A TAMAÑO GRANDE
                            if (C == 0)
                            {
                                J.addClass("selected")
                            }
                            J.attr("src", gthumbs[C]);// DIRECCION DE LA IMAGEN A CARGAR 
                            J.attr("data-large", glarge[C]);// DIRECCION DE LA IMAGEN A CARGAR 
                            P.append(J);  //sin estos tres elementos las imagenes no cargan.
                            I.append(P);  //sin estos tres elementos las imagenes no cargan.
                            i.append(I)  //sin estos tres elementos las imagenes no cargan.
                        }
                        ...
                        N.append(i).find(".related_photo").bind("click", function()
                        {
                            ...
                            $("<img/>").load(function() // PROPIEDAD QUE MUESTRA LA IMAGEN YA EXPANDIDA DEL SUBMENU
                            {
                                O.$fullimage.find("img").fadeOut(500, function()
                            {
                                    $(this).fadeIn(500).attr("src", $largePhoto)// PROPIEDAD QUE MUESTRA LA IMAGEN YA EXPANDIDA DEL SUBMENU
                                })
                            }).attr("src", $largePhoto)
                        });
                    ...
                    if (O.$fullimage.is(":visible"))
                    {
                        ...
                        $("<img/>").load(function()
                        {
                            ...
                            if (Q.attr("src") === O.$item.children("a").find("img").data("largesrc"))
                            {
                                ...
                                O.$largeImg = Q.fadeIn(350);//tiempo de aparición de la imagen principal (la más grande).
            ...
            calcHeight: function()
            {
                ...
                // alert(I +" - "+ i);
                this.height = I;
                this.itemHeight = i * 1;//número decimal, amplía el margen entre la expanción de imagen y los elementos debajo de él.
            },
            setHeights: function()
            {
                var i = this,
    ...
    function mostrarSection (argument) {
      $(".elastislide-list").load('planos_iseo.html');
    }
});